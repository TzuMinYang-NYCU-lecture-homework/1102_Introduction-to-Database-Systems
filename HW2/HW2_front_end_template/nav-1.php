  ...
  <div class="container">
    ...
    <div class="tab-content">
          ...
          <div class="  col-xs-8">
            <table class="table" style=" margin-top: 15px;">
              ...
              <?php
                ...
                {
                  if($_SESSION['order']=="shop_name")
                  {
                    foreach($_SESSION['n'] as $k=>$v)
                    {
                      $sid = $k;
                      $stmt = $conn->prepare("select shop_name, shop_category, st_distance_sphere(user_location, shop_location) as distance FROM shop, user where SID=:sid");
                  ...
                  else if($_SESSION['order']=="shop_category")
                  {
                  ...
                  else if($_SESSION['order']=="distance")
                  {
                    foreach($_SESSION['d'] as $k=>$v)
                    {
                      $sid = $k;
                      $stmt = $conn->prepare("select shop_name, shop_category, st_distance_sphere(user_location, shop_location) as distance FROM shop, user where SID=:sid");
                      $stmt->execute(array(':sid' => $sid));  # 防SQL injection
      ...
      <?php
        ...
        {
          # SQL查詢
          $stmt = $conn->prepare("select sid, shop_name, shop_category, ST_AsText(shop_location) as location FROM shop natrual join user where account=:account");
          $stmt->execute(array(':account' => $account));  # 防SQL injection
